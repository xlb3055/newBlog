...
* 🔐 **加密**：通信内容不可被窃听
* 🪪 **鉴权**：证明“你是谁”（用户 Key）
* 🆔 **身份校验**：证明“你连的是谁”（Host Key）

---

## 2️⃣ 两种完全不同的 Key（必须分清）
...
## 4️⃣ 端口转发（Port Forwarding）是什么

你使用的是：**SSH 本地端口转发（-L）**

### 一句话理解

> 把“内网的某个服务端口”，映射成“你本机的 localhost:端口”。
...
> **你只连跳板机一次，用 SSH 把整个内网“折叠”成 localhost。**
