...
* **可能有延迟**：比如我们 1s 拉一次，那最多可能延迟 1 秒才收到消息
...
> 如果我们设计一个系统，我们更倾向使用“拉 + 限流 + ACK”这种更稳定的方式。
...
选择哪种模式不是绝对，要看我们业务“更在意实时性”还是“更在意稳定性”。
...
- 消费者要支持限流与并发度控制
- Broker 需要有重试与死信队列
- 回调处理要可重入，避免重复投递造成副作用

### 拉模式的调度策略

- 拉取批次大小与频率要根据业务耗时动态调整
- 允许阻塞拉取，降低空轮询消耗
- 拉不到消息时的退避策略要有上限

### 一致性与幂等

- 业务侧要有幂等键，避免重复消费带来的重复写
- offset 提交要与业务处理成功绑定
