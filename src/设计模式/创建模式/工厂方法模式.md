...
## 角色与职责
- 产品（发送器）：产品接口，定义能力（发送消息）。
- 具体产品（短信发送器/邮件发送器）：具体产品实现。
- 创建者（消息服务）：定义工厂方法 `createSender()`，并在业务方法中使用产品。
- 具体创建者（短信消息服务/邮件消息服务）：决定创建哪种产品。

---

## 协作流程
1. 客户端 选择具体工厂（某个 `消息服务` 子类）。
...
怎么用：
- 把“通知发送器”抽象成 `发送器`，业务服务只依赖抽象。
- 每条业务线选择自己的 `消息服务` 子类，工厂方法返回对应渠道的发送器。
