...
- ConcreteIterator：保存遍历位置，负责具体遍历逻辑。
- Aggregate：定义创建迭代器的接口。
- ConcreteAggregate：具体集合，返回其对应的迭代器。

---

## 协作流程
1. Client 向 Aggregate 请求迭代器。
2. 迭代器保存遍历状态（索引/节点指针）。
3. Client 通过迭代器依次访问元素。
4. 遍历结束后，Client 停止访问。
...
怎么用：
- 仓储层对外只暴露 `iterator()`，内部决定是 List、分页查询还是游标。
